...
if len(sys.argv) > 1:
    root_dir = sys.argv[1] + '/'
# Create directories
if not os.path.exists(root_dir + 'include/GL'):
    os.makedirs(root_dir + 'include/GL')
if not os.path.exists(root_dir + 'src'):
    os.makedirs(root_dir + 'src')

# Download glcorearb.h
if not os.path.exists(root_dir + 'include/GL/glcorearb.h'):
    print('Downloading glcorearb.h to include/GL...')
    web = urllib2.urlopen('https://www.opengl.org/registry/api/GL/glcorearb.h')
    with open(root_dir + 'include/GL/glcorearb.h', 'wb') as f:
        f.writelines(web.readlines())
else:
    print('Reusing glcorearb.h from ' + root_dir + 'include/GL...')

# Parse function names from glcorearb.h
print('Parsing glcorearb.h header...')
procs = []
p = re.compile(r'GLAPI.*APIENTRY\s+(\w+)')
with open(root_dir + 'include/GL/glcorearb.h', 'r') as f:
    for line in f:
        m = p.match(line)
        if m:
...
# Generate gl3w.h
print('Generating gl3w.h in '+ root_dir + 'include/GL...')
with open(root_dir + 'include/GL/gl3w.h', 'wb') as f:
    f.write(UNLICENSE)
...
# Generate gl3w.c
print('Generating gl3w.c in '+ root_dir + 'src...')
with open(root_dir + 'src/gl3w.c', 'wb') as f:
    f.write(UNLICENSE)
